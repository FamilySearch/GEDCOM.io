...
| Python | MIT license | <https://github.com/vaelen/gedcom-lite>| A lightweight, fidelity-preserving parser for GEDCOM COM 5.5.1 and FamilySearch GEDCOM 7 |
...
| [maximal70-lds.ged](/testfiles/gedcom70/maximal70-lds.ged) | This file includes all of maximal70-tree1.ged plus the following structures: `BAPL`, `CONL`, `<<LDS_ORDINANCE_DETAIL>>`.`DATE`, `ENDL`, `INIL`, `SLGC`, `SLGS`, and `TEMP`. |
| [maximal70-memories1.ged](/testfiles/gedcom70/maximal70-memories1.ged) | This file includes all of maximal70-tree1.ged plus the following structures:  `INDI`.`OBJE`, `OBJE` (i.e., `<<MULTIMEDIA_RECORD>>`), `OBJE`.`FILE`, and `OBJE`.`FILE`.`FORM`. |
| [maximal70-memories2.ged](/testfiles/gedcom70/maximal70-memories2.ged) | This file includes all of maximal70-memories1.ged plus the following structures: `FAM`.`OBJE`, `<<EVENT_DETAIL>>`.`OBJE`, and `SOUR`.`OBJE`. |
| [maximal70-tree1.ged](/testfiles/gedcom70/maximal70-tree1.ged) | This file includes all of minimal70.ged plus the following structures: `ABBR`, `AUTH`, `BIRT`, `BURI`, `CHIL`, `CHR`, `<<EVENT_DETAIL>>`.`DATE`, `DEAT`, `FAM` (`<<FAMILY_RECORD>>`), `FAM`.`SOUR`, `INDI`.`FAMC`, `FAMS`, `FAM`.`HUSB`, `INDI` (`<<INDIVIDUAL_RECORD>>`), `INDI`.`SOUR`, `MARR`, `INDI`.`NAME`, `INDI`.`NOTE`, `PAGE`, `<<EVENT_DETAIL>>`.`PLAC`, `PUBL`, `SEX`, `SOUR` (`<<SOURCE_RECORD>>`), `SOUR`.`TITL`. |
| [maximal70-tree2.ged](/testfiles/gedcom70/maximal70-tree2.ged) | This file includes all of maximal70-tree1.ged plus many more `<<INDIVIDUAL_ATTRIBUTE_STRUCTURE>>`, `<<INDIVIDUAL_EVENT_STRUCTURE>>`, `<<FAMILY_ATTRIBUTE_STRUCTURE>>`, and `<<FAMILY_EVENT_STRUCTURE>>` structures, as well as `<<EVENT_DETAIL>>.SOUR`, `INDI`.`NAME`.`SOUR`, `NOTE`.`SOUR`, and `<<EVENT_DETAIL>>.NOTE`. |
