...
A `PHRASE` substructure contains "textual information that cannot be expressed in the superstructure due to the limitations of its data type." If the original source text is known, it would also be appropriate to use that text in the `PHRASE` instead of the 5.5.1 payload, such as:
```
2 DATE 30 JAN 1649
